...
template<typename T>
Node::BNode<T>::BNode(T data) : Node<T>(data), left(nullptr), right(nullptr), parent(nullptr) {}
...
template<typename T>
Node::BNode<T>::BNode(T data, bool red, bool black) : Node<T>(data), left(nullptr), right(nullptr), parent(nullptr), red(false), black(false) {}
...
{
    for (GraphNode<T> *a : this -> connectedNodes)
    {
        a -> printNode();
        std::cout << " ";
    }
